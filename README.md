...
That starts Vite on port `5179` and launches Electron (`OSMOS_DEV=1`).
...
- Heavy WASM (Whisper, Tesseract) stays in main or a system Node worker — never the renderer
- After main/preload changes, restart `npm run dev` (Vite HMR is not enough)
...
- Cloud keys (if any) stay in your local settings store — never commit them.
